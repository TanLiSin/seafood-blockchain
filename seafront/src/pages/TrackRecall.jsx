...
function TrackRecall() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/transactions-supplier-sender?username=${username}`);
  ...
  const submitFeedback = async () => {
    ...
    await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/notify-enduser`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        transaction_id: currentTxId,
  ...
  const supplierUser = localStorage.getItem('username');

  // Get all relevant transaction IDs
  const txIds = transactions.map(tx => tx.transaction_id);
  
  // Get feedback only tied to this supplier's transactions
  const feedbackForTx = allFeedback.filter(f => txIds.includes(f.transaction_id));
  
  // From those feedbacks, split into: supplier vs end user
  const supplierFeedback = feedbackForTx.filter(f => f.username === supplierUser);
  const endUserUsernames = [...new Set(transactions.map(tx => tx.end_user))];
  const endUserFeedback = feedbackForTx.filter(f => endUserUsernames.includes(f.username));  
    ...
    <div className="supplier-page">
      <header>
        ...
        <nav className="nav-links">
          ...
          <Link to="/track-recall" className="active">TRACK & RECALL</Link>
        </nav>
      ...
      <main>
        <div className="welcome-section">
                ...
                {filteredTransactions.length > 0 ? (
                  filteredTransactions.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td>{tx.freshness}</td>
                      <td>{tx.end_user || 'N/A'}</td>
                      <td>{tx.end_user_phone || 'N/A'}</td>
                      <td>
                        {tx.end_user_email ? (
                          <a
                            href={`https://mail.google.com/mail/?view=cm&to=${tx.end_user_email}`}
                            target="_blank"
          ...
          {showCharts && (
            ...
            <div style={{ width: '400px' }}>
            <div style={{ textAlign: 'center' }}>
              <h3>Feedback Ratio: Supplier vs End User</h3>
            </div>
            <Pie
            data={{
              labels: [
                `Supplier (${[...new Set(supplierFeedback.map(f => f.username))].join(', ') || 'N/A'})`,
                `End User (${[...new Set(endUserFeedback.map(f => f.username))].join(', ') || 'N/A'})`
              ],
              datasets: [{
                data: [supplierFeedback.length, endUserFeedback.length],
                backgroundColor: ['#0074d9', '#ffc107'],
                borderWidth: 1
              }]
            }}
          />
