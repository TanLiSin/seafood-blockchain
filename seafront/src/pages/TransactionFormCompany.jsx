...
function TransactionForm({ onSubmitSuccess, editTx, onCancelEdit }) {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/process-records?companyId=${companyId}`)
      .then((res) => res.json())
      .then(setProducts)
      .catch(console.error);

    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/distributors`)
      .then((res) => res.json())
      .then(setCompanies)
      .catch(console.error);
  }, []);
  ...
  useEffect(() => {
    if (editTx) {
      const expiryDateFormatted = editTx.expiry_date ? new Date(editTx.expiry_date).toISOString().split('T')[0] : '';
      setForm({
      ...
      setForm({ productId: '', freshness: '', amount: '', expiryDate: '', endUser: '', endUserWallet: '' });
    }
  }, [editTx]);
  ...
  const handleCompanySelect = (e) => {
    ...
    setForm({ ...form, endUser: username, endUserWallet: selectedCompany?.wallet_address || '' });
  };
  ...
  const proceedSubmit = async () => {
    ...
    try {
      if (editTx) {
        ...
        window.location.href = '/transaction-company';
      } else {
